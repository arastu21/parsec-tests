...
parsec_repo = Artifact.registerArtifact(
    ...
    cwd = './disk-image/',
    documentation = '''main repo to copy parsec source to the disk-image, the reason behind creating a directory with same name inside disk-image/parsec-benchmark is this way when the benchmark is copied to the disk-image they would not be scatted in ~ directory'''
)
